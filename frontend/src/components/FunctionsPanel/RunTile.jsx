...
import { getNextState, getJob, stopJob } from '../../api.js';
import PipelineOutput, {
  ...
  PrePipelineOutput,
  PostPipelineOutput,
} from '../JobsPanel/PipelineOutput.jsx';
...
export function MultiSelector({ value, onChange, names, placeholder }) {
  ...
  function handleKeyDown(e) {
    ...
    else if (e.key === 'Escape') { if (expanded) setExpanded(false); else { setOpen(false); setHighlightedIndex(-1); } }
  }
    ...
    <div className="flex flex-col gap-1">
      <div className="relative" ref={containerRef}>
        ...
        {open && filtered.length > 0 && !expanded && (
          <div className="absolute top-full left-0 right-0 z-10 mt-0.5 bg-popover border border-border rounded-md shadow-lg">
            ...
            <div ref={listRef} className="max-h-44 overflow-y-auto">{listItems}</div>
          </div>
        )}
      </div>
      {expanded && open && filtered.length > 0 && createPortal(
        <div style={{ position: 'fixed', top: 0, bottom: 0, left: expandedRect?.left ?? 0, width: expandedRect?.width ?? 300, zIndex: 9999 }}
          ...
          <div ref={listRef} className="flex-1 overflow-y-auto">{listItems}</div>
        </div>,
        document.body,
      )}
      {value.length > 0 && (
...
export function StateSelector({ value, onChange, stateNames }) {
  ...
  function selectState(state) { onChange(state); setSearch(''); setOpen(false); setExpanded(false); setHighlightedIndex(-1); }
  function handleInput(e) { onChange(''); setSearch(e.target.value); setOpen(true); setHighlightedIndex(-1); }
