...
## Fix 9 — Cross-crop keyword exclusion in Stage 6

**Files:** `pipeline/filter_faq_corpus.py`, `run_pipeline.py`  
**Stage:** Stage 6 — Corpus Filter  
**Severity:** Enhancement — closes a gap where cross-crop contamination surviving Stage 3B was not caught at the final FAQ stage

**Root cause / gap:**  
Stage 6 only used `irrelevant_corpus.yaml` (weather, market prices, contact numbers). Questions
about a different crop that slipped through Stage 3B's LLM filter reached `unique_questions_freq.csv`
unchecked. `crops.yaml` (248 crops, each with ~20 keywords) existed but was not wired into the
pipeline anywhere.

**Changes — `pipeline/filter_faq_corpus.py`:**

Added helpers `_normalize_crop_name` and `_find_crop_key` for matching the pipeline's
`--crop` argument (e.g. `"Maize (Makka)"`) to a `crops.yaml` key (e.g. `"maize"`).

Added `load_cross_crop_keywords(crops_yaml, target_crop) -> list[str]`:
```python
own_keywords = { k.lower().strip() for k in crops[target_key].get('keywords', []) ... }
other_keywords = set()
for key, body in crops.items():
    if key == target_key:
        continue
    for kw in body.get('keywords', []):   # keywords only, not typos
        other_keywords.add(kw.lower().strip())
exclusion = other_keywords - own_keywords  # never exclude target crop's own terms
```

Modified `filter_faq()` to accept `extra_keywords: list[str] | None = None`:
```python
if extra_keywords:
    keywords = list(set(keywords) | set(extra_keywords))
```

Added `--crops-file` and `--crop` CLI args to `main()`.

**Changes — `run_pipeline.py`:**

`run_corpus_filter()` now accepts `crop` and `crops_yaml` parameters:
```python
def run_corpus_filter(out_dir, corpus_file, fuzz_thresh=85,
                      crop=None, crops_yaml=None):
    ...
    if crop and crops_yaml and Path(crops_yaml).exists():
        extra_keywords = load_cross_crop_keywords(crops_yaml, crop)
    kept_df, removed_df = filter_faq(..., extra_keywords=extra_keywords)
```

Added `--crops-file` CLI arg (defaults to `crops.yaml` in project root if present).  
Stage 6 call updated to pass `crop=args.crop, crops_yaml=args.crops_file`.

**Key design decision — keywords only, not typos:**  
`crops.yaml` also contains a `typos` list per crop (generated by `generate_typos.py`).
These are intentionally excluded from the exclusion set. Using typos of other crops as
exclusion criteria would cause false positives (e.g. "whet" is a valid English word, not
only a typo for "wheat"). The existing `is_irrelevant()` fuzzy-match fallback provides a
safety net for obvious misspellings without needing the typos list.

**Key design decision — subtract target crop's own keywords:**  
Any keyword shared between the target crop and another crop is removed from the exclusion set:
`exclusion = other_keywords - own_keywords`. This prevents a question legitimately about the
target crop from being filtered out just because a shared term (e.g. "blast disease") appears
under another crop's entry too.
